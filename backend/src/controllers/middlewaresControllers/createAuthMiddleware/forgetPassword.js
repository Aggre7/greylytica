...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const greylytica_app_email = settings['greylytica_app_email'];
  const greylytica_base_url = settings['greylytica_base_url'];

  const url = checkAndCorrectURL(greylytica_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | greylytica',
